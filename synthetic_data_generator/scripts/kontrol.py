...
# Çoklu veri üretimi için yeni deney ID'si
EXPERIMENT_ID = 5  

# Sınıf ID Eşleştirmeleri (4 Sınıf)
TARGET_OBJECTS = {
...
# ==================== ÇOKLU ANOMALİ SENARYOLARI ====================
SCENARIOS = [
    {
        "name": "rock",
        "target_obj": "Rock",        # Bu aralıkta SADECE Rock etiketlenecek
        "start_frame": 1,
        "end_frame": 145
    },
]
...
print(f"=== KONTROLLÜ ANOMALİ VERİ ÜRETİMİ BAŞLADI (EXP {EXPERIMENT_ID}) ===")

for scenario in SCENARIOS:
    scenario_name = scenario["name"]
    target_obj_name = scenario["target_obj"]  # <-- Hangi nesnenin etiketleneceği buraya bağlandı
    start_frame = scenario["start_frame"]
    end_frame = scenario["end_frame"]
    
    print(f"\n--- İşleniyor: {scenario_name} | Hedef Nesne: {target_obj_name} (Kareler: {start_frame} - {end_frame}) ---")
    ...
    for frame in range(start_frame, end_frame + 1, FRAME_STEP):
        ...
        # 5. SADECE SENARYODA BELİRTİLEN HEDEF NESNE İÇİN ETİKET HESAPLA
        annotations = []
...
print(f"\n=== KONTROLLÜ ANOMALİ VERİ ÜRETİMİ BAŞARIYLA TAMAMLANDI! ===")
